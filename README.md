...
$M_g$ = 24,576 MiB (RTX 3090). $R_g$ = CUDA context overhead: 350 to 450 MiB per GPU (driver page tables, command buffers, ECC structures). Display connected: add 200 to 500 MiB.
...
GPU 1 (middle card) runs 8 to 12°C hotter than edge cards at the same load, sandwiched between two heat sources with restricted airflow. Thermal throttle at 83°C silently reduces clock speed, adding 15 to 20% inference latency.
